...
class CommentSystem {
  ...
  async deleteComment(commentId) {
    ...
    // Prevent multiple deletions of the same comment
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Comment already deleted, just reload comments
        console.log('Comment already deleted, refreshing list');
        await this.loadComments();
      } else {
        const error = await response.json();
...
// Global comment system manager to prevent multiple initializations
window.commentSystemManager = {
  ...
  destroy: function() {
    if (this.currentSystem) {
      // Properly destroy the comment system
...
// Initialize comment system when DOM is ready (backward compatibility)
